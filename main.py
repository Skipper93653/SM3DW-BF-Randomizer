...
def randomizer():
    ...
    fileName = 'StageList-'+currentDateTime+'.txt'

    os.system('cmd /c "mkdir romfs-'+currentDateTime+'"')

    os.system('cmd /c "copy StageList.txt '+fileName+'"')
    os.system('cmd /c "move '+fileName+' romfs-'+currentDateTime+'"')

    with open('StageList.txt', encoding='utf-8') as old:
        StageListOld = [line for line in old.readlines()]

    with open('./romfs-'+currentDateTime+'/'+fileName, encoding='utf-8') as new:
        StageListNew = [line for line in new.readlines()]
    ...
    while stageNo <= 154:
        ...
        # This loop happens when stageID generates a special level when stageNo is currently a castle stage and forces stageID to randomize again until it's not a special level.
        ...
        StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 1] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageID) + '\n')) + 1]  # DoubleMarioNum
        # Only have Bowser-Castle keep it's Star Lock of 170.
        if StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n'))] == '        - CourseId: !l 114\n':
            print('Reached Bowser-Castle slot! Placing 170 star lock!')
            StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 4] = '          GreenStarLock: !l 170\n'
        else:
            StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 4] = '          GreenStarLock: !l 0\n'  # GreenStarLock - gets rid of all star locks by setting it to 0.
        StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 5] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageID) + '\n')) + 5]  # GreenStarNum
        StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 6] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageID) + '\n')) + 6]  # IllustItemNum
        StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageID) + '\n')) + 8]  # StageName

        if 'クッパ城' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10]:
            StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10]  # Makes sure all castle slots are the same slots when the levels are randomized.
        else:
            # Making sure Captain Toad stages, Toad Houses, Stamp Houses, Roulettes, Blockades, and Golden Express have the correct StageType.
            if 'KinopioBrigade' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                print('Captain Toad StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: キノピオ探検隊\n'  # StageType for Captain Toad levels.
            elif 'KinopioHouse' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                print('Toad House StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: キノピオの家\n'  # StageType for Toad Houses.
            elif 'FairyHouse' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                print('Stamp House StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: 妖精の家\n'  # StageType for Stamp Houses.
            elif 'RouletteRoomZone' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                print('Roulette StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: キノピオの家\n'  # StageType for Roulettes (カジノ部屋) is not used because they don't appear on the world map immediately which can cause progression issues. So Toad House is used instead.
            elif 'GoldenExpressStage' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                print('Golden Express StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: ゴールデンエクスプレス\n'  # StageType for golden express.
            elif 'GateKeeper' in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                if 'GateKeeperBoss' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] or 'GateKeeperTentack' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8]:
                    ...
                    StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: 通常\n'
            elif ('KinopioBrigade' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: キノピオ探検隊\n') or ('KinopioHouse' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and (StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: キノピオの家\n' or StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: 隠しキノピオの家\n' or StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: 隠し土管')) or ('FairyHouse' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: 妖精の家\n') or ('RouletteRoomZone' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: カジノ部屋\n') or ('GoldenExpressStage' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and (StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: ゴールデンエクスプレス\n')) or ('GateKeeper' not in StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 8] and (StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: ゲートキーパー[GPあり]\n' or StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] == '          StageType: ゲートキーパー\n')):
                print('Non-\'special\' level with \'special\' StageType fixed!')
                StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 10] = '          StageType: 通常\n'  # StageType for normal levels.
        StageListNew[(StageListNew.index('        - CourseId: !l ' + str(stageNo) + '\n')) + 9] = StageListOld[(StageListOld.index('        - CourseId: !l ' + str(stageID) + '\n')) + 9]  # StageTimer
        stageNo += 1
    ...
    print('If no indication of duplicate numbers are above, success!')

    with open('./romfs-'+currentDateTime+'/'+fileName, 'w', encoding='utf-8') as f:
        f.write(''.join([''.join(l2) for l2 in StageListNew]))
...
def spoilerFile(StageListNew, currentDateTime):
    ...
    with open('./romfs-'+currentDateTime+'/'+currentDateTime+'-spoiler.txt', 'w', encoding='utf-8') as s:
        s.write(''.join([''.join(l2) for l2 in stageID_Name]))
...
# Place check boxes
spoilerCheck.place(relx=0.01, rely=0.5, anchor=W)
